...
echo "width,elements,num,extra_triples,tdId,co_decision,monolithic_decision,consistent"
...
for f in $@; do
	...
	IFS="_/" read width elements num triples tdId <<< "$instance_data"
	echo -n "$width,$elements,$num,$triples,$tdId,"

	unset consistent

	# DECISION
	read time exitCode <<< $(\time -f "%U %S %x" bash -c "$dflat -p decision -e order -x $exchangeDecEnc -s $tdId < $f" 2>&1 | tail -n1 | awk '{printf("%s %s", $1+$2, $3)}')
